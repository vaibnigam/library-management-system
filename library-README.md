...
This project is the first in a three-part series exploring layered Java design with progressively richer storage and business logic. It deliberately uses raw array storage rather than the Collections framework used in the later projects, to build a solid, first-principles understanding of manual indexing, linear search, and array compaction before those mechanics are abstracted away by `HashMap` and `List`.
